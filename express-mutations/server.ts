...
app.post('/api/actors', async (req, res, next) => {
  try {
    ...
    if (!firstName) {
      throw new ClientError(400, 'firstName undefined');
    } else if (!lastName) {
      throw new ClientError(400, 'lastName undefined');
    }
    const sql = `
...
app.put('/api/actors/:actorId', async (req, res, next) => {
  try {
    ...
    if (!Number.isInteger(+actorId)) {
      ...
      throw new ClientError(400, 'Undefined firstName');
    } else if (!lastName) {
      throw new ClientError(400, 'Undefined lastName');
    }
    const sql = `
      UPDATE "actors"
...
app.delete('/api/actors/:actorId', async (req, res, next) => {
  try {
    const { actorId } = req.params;
    if (!Number.isInteger(+actorId)) {
      throw new ClientError(400, 'actorId undefined');
    }
    const sql = `
      DELETE FROM "actors"
      WHERE "actorId" = $1
