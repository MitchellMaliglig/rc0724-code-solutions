...
function App() {
  ...
  function handleClickTopic(e: React.MouseEvent) {
    const selectedTopic = (e.target as HTMLParagraphElement).textContent;

    if (selectedTopic) {
      selectedTopic !== topic ? setTopic(selectedTopic) : setTopic('');
    }
  }
  ...
  return (
    <>
      <Accordion
        topics={topics}
        currentTopic={topic}
        onClickTopic={handleClickTopic}></Accordion>
    </>
  );
}
